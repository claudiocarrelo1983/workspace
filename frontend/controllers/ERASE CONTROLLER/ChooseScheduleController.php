...
class ChooseScheduleController extends Controller
{
    ...
    {      
        ...
        if(Yii::$app->request->get('day') != '*') {
            $validateDay = Helpers::checkDate($day);

            if($validateDay == false){
                return $this->redirect(['/choose-location',                
                    'location' => Yii::$app->request->get('location'),
                    'code' => Yii::$app->request->get('code'),  
                    'team' => Yii::$app->request->get('team'),                  
                    'service' =>  $model->service_code,
        ...
        if(isset(Yii::$app->request->post()['date'])){
            ...
            if (strtotime($today) > strtotime($date)) {
                ...
                return $this->redirect(['/choose-schedule',                
                    'location' => Yii::$app->request->get('location'),
                    'code' => Yii::$app->request->get('code'),  
                    'team' => Yii::$app->request->get('team'),                  
                    'service' =>  Yii::$app->request->get('service'),                   
                    'day' => strtotime(Yii::$app->request->post()['date']),
                    'time' => Yii::$app->request->get('time')      
                    ]
                );  
            }
        ...
        */
        ...
        if (Yii::$app->request->isPost && $model->load(Yii::$app->request->post())) {          
            ...
            $date = date('Y-m-d',Yii::$app->request->get('day'));
            $time = date('H:i',$model->time);
          
           
            if(strtotime($date.' '.$time) < time()){    
                Yii::$app->session->setFlash('error', Yii::t('app', 'schedule_invalid'));        

            }else{
                return $this->redirect(['/choose-details',                
                    'location' => Yii::$app->request->get('location'),
                    'code' => Yii::$app->request->get('code'),  
                    'team' => Yii::$app->request->get('team'),                  
                    'service' =>  Yii::$app->request->get('service'),          
        ...
        return $this->render('@frontend/views/client/client-booking/schedule', [             
            'model' => $model, 
    ...
    {        
        ...
        if (Yii::$app->request->isPost && $model->load(Yii::$app->request->post())) {           
            ...
            $model->service_name = '1';  
            $model->client_username = '1';                
    ...
    public function formCreate($model){
